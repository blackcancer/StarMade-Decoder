...
describe('Inventory — toTag with meta ItemStack', () => {
  it('toTag with meta round-trip', () => {
    ...
    const tag = inv.toLegacyTag();
    assert.throws(() => inv.toTag(), 'payload');
    assert.throws(() => Inventory.fromTag(tag), 'fromLegacyTag');
    const rt = Inventory.fromLegacyTag(tag);
...
describe('SmbplParser — complex connection format', function() {
  ...
  it('parseSmbpl with minimal header (connectionCount=0)', () => {
    ...
    w.writeInt32BE(-1026); // disk controller map v2
