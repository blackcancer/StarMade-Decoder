...
export function copyInventoryTag(tag: Tag): Tag { return readFrom(writeTo(tag)); }
...
function content(tag: Tag, parents: Tag[] = []): { tag: Tag; parents: Tag[] } {
  if (parents.length > 16) throw new DecodeError('E_LIMIT', 'Inventory envelope nesting exceeds 16');
...
function decodeItem(slot: number, type: number, value: Tag): InventoryWireItem {
  ...
  return { slot, type, count: 1, meta: { id: parts[0].getInt(), type, orientation: 0,
    subId: parts[3]?.type === TagType.SHORT ? parts[3].getShort() : -1, payload: copyInventoryTag(parts[2]) } };
}

/** Recognizes actual format variants; anonymous legacy SDK tuples return undefined for compatibility. */
export function readInventoryWire(tag: Tag, maxSlots = Infinity): InventoryWireData | undefined {
  ...
  const template = writeTo(tag), inner = content(readFrom(template)).tag, data = children(inner);
  ...
  const seen = new Set<number>(), items = slots.map((slot, index) => {
    ...
    seen.add(value); return decodeItem(value, types[index].getShort(), values[index]);
  });
...
function encodeItem(item: ItemStack, previous?: Tag): Tag {
  if (item.group) {
  ...
  if (meta?.payload) {
    ...
    data[0] = scalar(data[0], Tags.int(null, meta.id)); data[1] = scalar(data[1], Tags.short(null, meta.type)); data[2] = copyInventoryTag(meta.payload);
    if (data[3]?.type === TagType.SHORT || meta.subId !== -1) data[3] = scalar(data[3], Tags.short(null, meta.subId));
...
export function writeInventoryWire(items: readonly ItemStack[], template?: Buffer): Tag {
  const source = template ? content(readFrom(template)) : undefined;
  ...
  for (const item of sorted) {
    ...
    values.push(encodeItem(item, previous.get(item.slot)));
  }
